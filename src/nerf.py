...
class NeRFWrapper(nn.Module):
    def __init__(self, settings: Settings):
        ...
        # encoders
        encoder = PositionalEncoder(self.ss.d_input, self.ss.n_freqs, log_space=self.ss.log_space)
    ...
    def forward(self,
        ...
        query_points, z_vals = stratified_sample(rays_o, rays_d, self.ss.near, self.ss.far, **self.ss.kwargs_sample_stratified)
        # Prepare batches.
        ...
        if self.viewdirs_encoding_fn is not None:
            batches_viewdirs = self.prepare_viewdirs_chunks(query_points, rays_d, self.viewdirs_encoding_fn, chunksize=self.ss.chunksize)
        else:
